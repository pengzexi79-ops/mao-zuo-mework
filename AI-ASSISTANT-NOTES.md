...
| MySQL 8 + 数据(schema 19 表) | `portable\mysql` + `portable\mysqldata` | ✅ 用户 mixcut；密码需按本机配置 |
...
8. **ensure_env.bat + .env.example(16:30)** — **新电脑连不上内置 MySQL 的关键 bug**:两处默认数据库配置需要与安装包内置 `portable\mysqldata` 的 mixcut 用户保持一致；分享快照已删除真实密码，需按目标机器配置后再构建安装器。
